...
public:
  ...
  SVGDesc(SvgStreamPtr stream_, bool standalone_, cpp11::list aliases_,
          ...
          bool fix_text_size_, double scaling_, bool always_valid_) : stream(stream_),
                                                                      pageno(0),
                                                                      is_inited(false),
                                                                      is_clipping(false),
...
void svg_clip(double x0, double x1, double y0, double y1, pDevDesc dd)
{
  ...
  s << dbl_format(xmin) << "|" << dbl_format(xmax) << "|" << dbl_format(ymin) << "|" << dbl_format(ymax);
  ...
  if (!stream->has_clip_id(clipid))
  {
    ...
    (*stream) << "    <rect x='" << xmin << "' y='" << ymin << "' width='" << (xmax - xmin) << "' height='" << (ymax - ymin) << "' />\n";
    (*stream) << "  </clipPath>\n";
    (*stream) << "</defs>\n";
  }
              ...
              const pGEcontext gc, pDevDesc dd)
{
  ...
  if (svgd->is_recording_clip)
  {
    (*stream) << "M " << x0 << ',' << y0 << " L " << x0 << ',' << y1 << " L " << x1 << ',' << y1 << " L " << x1 << ',' << y0;
    stream->put('Z');
    return;
  }
  if (isfirst == true)
  ...
  (*stream) << "<rect x='" << fmin(x0, x1) << "' y='" << fmin(y0, y1) << "' width='" << fabs(x1 - x0) << "' height='" << fabs(y1 - y0) << '\'';
  write_attr_mask(stream, svgd->current_mask);
  write_style_begin(stream);
  write_style_linetype(stream, gc, svgd->scaling, true);
  write_style_fill(stream, gc);
...
void bamberg_circle(double x, double y, double r, const pGEcontext gc,
                    pDevDesc dd)
{
  SVGDesc *svgd = (SVGDesc *)dd->deviceSpecific;
  if (!svgd->is_inited)
  ...
  if (svgd->is_recording_clip)
  {
    (*stream) << "M " << x - r << ',' << y << " a " << r << ',' << r << " 0 1,1 " << r * 2 << ",0 "
              << " a " << r << ',' << r << " 0 1,1 " << -r * 2 << ",0 ";
    stream->put('Z');
    return;
  }
...
[[cpp11::register]] bool svglite_(std::string file, std::string bg, double width, double height,
                                  double pointsize, bool standalone, cpp11::list aliases,
...
[[cpp11::register]] cpp11::external_pointer<std::stringstream> svgstring_(cpp11::environment env, std::string bg,
...
[[cpp11::register]] std::string get_svg_content(cpp11::external_pointer<std::stringstream> p)
{
  p->flush();
